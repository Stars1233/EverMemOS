...
import { EverMemOSService } from './services/EverMemOSService.js';
...
logger.info('Server', `Memory service: ${USE_EVERMEMOS ? (isCloudMode ? 'EverMind Cloud' : 'EverMemOS (local)') : 'Mock'}`);
if (USE_EVERMEMOS) {
  logger.info('Server', `EverMemOS URL: ${EVERMEMOS_URL}`);
  if (isCloudMode) {
