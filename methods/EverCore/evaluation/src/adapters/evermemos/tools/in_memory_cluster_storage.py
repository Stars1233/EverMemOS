...
during MemCell extraction. It keeps mem scene states in memory with optional
...
class InMemoryClusterStorage:
    ...
    async def save_mem_scene(self, group_id: str, state: Dict[str, Any]) -> bool:
        """Save mem scene state for a group."""
        try:
            ...
            logger.error(f"Failed to save mem scene for group {group_id}: {e}")
            return False

    async def load_mem_scene(self, group_id: str) -> Optional[Dict[str, Any]]:
        """Load mem scene state for a group."""
    ...
    async def get_cluster_assignments(self, group_id: str) -> Dict[str, str]:
        ...
        Supports both new and old serialization formats:
        - New: { "memcell_info": { memcell_id: { "memscene": cluster_id, ... } } }
        - Old: { "eventid_to_cluster": { event_id: cluster_id } }
        """
        state = self._states.get(group_id, {})
        if "memcell_info" in state:
            return {
                memcell_id: str(info["memscene"])
                for memcell_id, info in state["memcell_info"].items()
                if "memscene" in info
            }
        return state.get("eventid_to_cluster", {})

    async def clear(self, group_id: Optional[str] = None) -> bool:
        """Clear mem scene state."""
        try:
            if group_id is None:
                self._states.clear()
            ...
            logger.error(f"Failed to clear mem scene state: {e}")
            return False
    ...
    def _persist_to_disk(self, group_id: str, state: Dict[str, Any]) -> None:
        """Persist mem scene state to JSON file."""
        if not self._persist_dir:
            return

        try:
            state_file = self._persist_dir / f"mem_scene_{group_id}.json"
            ...
            logger.warning(f"Failed to persist mem scene for group {group_id}: {e}")

    def _load_from_disk(self) -> None:
        """Load mem scene states from disk on initialization."""
        if not self._persist_dir or not self._persist_dir.exists():
            return

        try:
            for state_file in self._persist_dir.glob("mem_scene_*.json"):
                try:
                    group_id = state_file.stem.replace("mem_scene_", "")
                    with open(state_file, "r", encoding="utf-8") as f:
                        state = json.load(f)
                    self._states[group_id] = state
                    logger.info(f"Loaded mem scene for group {group_id} from disk")
                except Exception as e:
                    logger.warning(f"Failed to load mem scene from {state_file}: {e}")
        except Exception as e:
            logger.error(f"Failed to load mem scenes from disk: {e}")
