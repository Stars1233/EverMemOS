# Team Chat Conversations Guide

[Home](../../README.md) > [Docs](../README.md) > [Advanced](.) > Team Chat Guide

## Overview

EverCore supports organizing conversations into **groups** using `group_id` and `group_name`. This allows you to:
...
| `scene` | No | Scene type: `solo` (1:1 with AI) or `team` (group chat) |
...
Create a JSON file following the ConversationFormat specification.

### Step 2: Process the Group Chat

```bash
uv run python src/bootstrap.py src/run_memorize.py \
  --input your_team_chat.json \
  --scene team \
  --api-url http://localhost:1995/api/v0/memories
```

**Parameters:**
- `--input`: Path to your ConversationFormat JSON file (required)
- `--scene`: Memory extraction scene - `team` or `solo` (required)
...
| `atomic_fact` | Atomic facts extracted from episodes |

---

## Generating Group Summaries

EverCore can generate contextual summaries within a group because it understands:
...
response = requests.get(
    ...
    json={
        ...
        "group_id": "team_001",
        "memory_types": ["episodic_memory", "atomic_fact"],
...
- Use `team` for multi-person group chats
- Use `solo` for 1:1 conversations with an AI assistant
...
- **[Chinese Sample](../../data/team_chat_zh.json)** - Chinese language example
- **[English Sample](../../data/team_chat_en.json)** - English language example
- **[Format Specification](../../data_format/conversation/conversation_format.md)** - Complete format reference

---

## See Also

- [Conversation Format Specification](../../data_format/conversation/conversation_format.md) - Complete data format reference
